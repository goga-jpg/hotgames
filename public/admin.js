// Admin UI state. Everything is derived from these four values; when any of
// them changes we rebuild the preview URL and the ad-tag snippet.
const state = {
  fixtures: [],
  sizes: [],
  matchId: null,
  sizeId: null,
  layout: null,     // { staticRegion:{}, dynamic:{ slot: {x,y,w,h, align} } }
  baseLayout: null  // preset returned by the server, used for "reset"
};
...
async function boot() {
  const [sizes, fixtures] = await Promise.all([
  ...
  fillOptions($("size"), sizes, (s) => s.id, (s) => `${s.name} · ${s.w}×${s.h}`);
  fillOptions($("fixture"), fixtures, (f) => f.id,
    (f) => `${f.home.name} vs ${f.away.name} · ${new Date(f.kickoff).toUTCString().slice(5, 22)}`
  );

  state.sizeId = sizes[0]?.id || null;
  state.matchId = fixtures[0]?.id || null;
  ...
  $("refreshFixtures").addEventListener("click", refreshFixtures);

  await loadLayoutAndRender();
  updateMatchMeta();
}

async function loadLayoutAndRender() {
  const info = await fetch(`/api/sizes/${encodeURIComponent(state.sizeId)}`).then((r) => r.json());
...
function renderPlacementTable() {
  ...
  for (const elDef of ELEMENTS) {
    const box = elDef.staticRegion
      ? state.layout.staticRegion
      : state.layout.dynamic[elDef.key];
...
function refreshPreview() {
  const size = state.sizes.find((s) => s.id === state.sizeId);
  if (!size) return;
  ...
  // Build a stable embed snippet (no layout param → falls back to preset).
  fetch(`/api/adtag?size=${encodeURIComponent(state.sizeId)}&match=${encodeURIComponent(state.matchId || "")}`)
    .then((r) => r.json())
    .then((a) => {
      $("snippet").value = a.snippet;
      $("openCreative").href = a.url;
    });
}
...
async function refreshFixtures() {
  ...
  if (!fixtures.find((f) => f.id === state.matchId)) {
    state.matchId = fixtures[0]?.id || null;
    $("fixture").value = state.matchId;
  }
  updateMatchMeta();
